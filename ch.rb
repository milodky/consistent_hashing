...
class ConsistentHashing
  attr_reader :hash_tuples
  MAX_NUM = 1000000
  def hash_key(key)
    (Digest::MurmurHash1.rawdigest(key) % MAX_NUM) / MAX_NUM.to_f
  end
  def initialize(num_machines = 1, num_replicas = 1)
    @num_machines = num_machines
    @num_replicas = num_replicas
    @hash_tuples  = []
    (0...@num_machines).each do |j|
      (0...@num_replicas).each do |k|
        @hash_tuples << [j, k, hash_key("#{j}_#{k}")]
      end
    end
    @hash_tuples.sort!{|a, b| a[2] <=> b[2]}
  end

  def get_machine(key)
    h = hash_key(key)
    vms = @hash_tuples.each do |j, _, v|
      return j if h < v
    end
    @hash_tuples[0][0]
  end
...
def run
  ch = ConsistentHashing.new(7, 3)
  puts "Format:"
  puts "machine\treplica\thash value"
  ch.hash_tuples.each do |tuple|
    puts tuple.join("\t")
  end
  while true
    key = gets.chomp
    puts "key\tmachine\thash key"
    puts "#{key}\t#{ch.get_machine(key)}\t#{ch.hash_key(key)}"
  end
